...
async (page) => {
 ...
 try{
  ...
  for(const id of ['wilton-two-room-82','wilton-three-room-92','wilton-four-room-122']){
   await page.goto(`http://127.0.0.1:4173/Artwin/#/projects/wilton-park/apartments/${id}`);await page.reload();
   ...
   for(let i=0;i<stops;i++){
    ...
    if(i===0||i===1||i===stops-2)await page.screenshot({path:`output/playwright/${id}-tour-${i}.png`});
   }
  ...
  check(await page.locator('.residence-card').count()===3,'Wilton gallery does not show three plans');
  for(const locale of ['ru','ky','zh-CN','en-US']){
   await page.locator('.collection-header .language-picker select').selectOption(locale);
   check(await page.locator('.residence-card').count()===3,'Language switch lost plans');
  }
  ...
  await page.locator('.residence-card').first().scrollIntoViewIfNeeded();await page.screenshot({path:'output/playwright/wilton-gallery-mobile.png'});
  ...
  await page.screenshot({path:'output/playwright/wilton-tour-mobile.png'});
