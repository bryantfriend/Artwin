...
export default function Joystick({input,disabled}) {
  ...
  return <div className="joystick" role="group" aria-label="Touch movement joystick" onPointerDown={e=>{if(disabled)return;pointer.current=e.pointerId;e.currentTarget.setPointerCapture(e.pointerId);update(e);}} onPointerMove={update} onPointerUp={release} onPointerCancel={release} onLostPointerCapture={release}>
    <span style={{transform:`translate(${knob.x}px,${knob.y}px)`}}/><small>MOVE</small>
  </div>;
}
