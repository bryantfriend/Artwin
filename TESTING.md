...
## Compact mobile floor-plan gallery — 14 September 2026

Mobile cards collapse metrics, highlights, descriptions and comparison controls behind a translated disclosure button. The focused browser check passed at 320, 390, 600 and 760 px in all four languages: collapsed cards measured 258–280 px in the recorded first-card samples, and every card stayed below 310 px. It checked independent expansion, collapse, preserved comparison selections, 44 px touch targets, enlarged previews/focus restoration, desktop detail visibility and no horizontal overflow. Collapsed and expanded screenshots were reviewed.

All 38 Node tests, the production build and strict `/Artwin/` asset check passed. The full collection regression also passed filtering, sorting, favorites, comparison, language changes and responsive desktop/mobile layouts. Both browser runs reported no console/runtime errors or missing assets. Checks use resized desktop Chrome viewports, not physical phones.

## Buyer platform — 14 September 2026

- 46 Node tests pass, including payment math (zero-interest, interest, cash, invalid input), inventory schema/identity/status/link/date validation, matching with missing prices and mixed currencies, share-route allowlisting, contextual inquiry escaping, opt-in measurement privacy, approximate furniture boundary checks and translated project facts.
- Production Vite build and strict `/Artwin/` server asset checks pass. The production check also requests and validates `sales-data.json`; unknown URLs still return 404.
- `scripts/browser-buyer-tools.js`: calculator and quarterly schedule, invalid contribution, saved scenario, contextual WhatsApp URL, furniture sizing/rotation, no invented inventory, shared link reopening/private-note exclusion, guided matching, and 320/390/760/1440 layouts in all four languages.
- `scripts/browser-sales-workspace.js`: imports `tests/fixtures/sales-inventory.json` only into the isolated test browser, filters building/floor/status, inspects unit-specific inquiries and dated records, verifies local opt-in counts, removes the preview and confirms the public inventory remains empty. Shared sales outcomes remain disconnected. All four languages checked for overflow.
- `scripts/browser-buyer-viewer.js`: on-demand map rendered, category and directions URLs checked, viewer purchase menu/back button at 320/390 in all languages, context and furniture access, Escape focus restoration and paused walkthrough after closing the tools. No app console errors; existing Three.Clock and Rapier initialization deprecation warnings remain.
- Screenshots are in ignored `output/playwright/buyer-*.png` and were visually reviewed. Browser scripts use the repository's existing Playwright CLI `run-code --filename` workflow against `npm run serve:pages`.
