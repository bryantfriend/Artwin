...
async(page)=>{
 ...
 try{
  await page.setViewportSize({width:1440,height:960});await page.goto(base);await page.getByRole('heading',{name:'Find your place. Imagine your life.'}).waitFor();
  ...
  for(const name of projects){
   ...
   check(await page.getByRole('link',{name:'Explore in 3D',exact:true}).count()===(name==='Tokyo City'?1:0),'Incorrect plan availability for '+name);
  ...
  await page.getByRole('link',{name:'Explore Tokyo City',exact:true}).first().click();const projectUrl=page.url();await page.reload();await page.getByRole('heading',{name:'Tokyo City',exact:true,level:1}).waitFor();
  await page.getByRole('link',{name:'Explore in 3D',exact:true}).click();await ready();await page.getByRole('combobox',{name:'Graphics quality'}).selectOption('low');check(page.url().includes('#/projects/tokyo-city/apartments/four-room-134'),'Apartment route wrong');
