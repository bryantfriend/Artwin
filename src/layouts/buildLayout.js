...
import {pointInPolygon,circleIntersectsBox,wallSegments,doorPose,wallPoint,wallYaw} from '../geometry.js';
...
function makeWalls(rooms,openings){
 ...
 for(const room of rooms)for(let i=0;i<room.polygon.length;i++){
  ...
  if(a[0]!==b[0]&&a[1]!==b[1]){diagonals.push({a,b,room:room.id});continue;}
 ...
 // Bay edges retain their real angle in the plan, render and collision geometry.
 for(const {a,b,room} of diagonals){
  const length=Math.hypot(b[0]-a[0],b[1]-a[1]),yaw=-Math.atan2(b[1]-a[1],b[0]-a[0]);
  const mid=[(a[0]+b[0])/2,(a[1]+b[1])/2],owner=rooms.find(r=>r.id===room);
  const normal=pointInPolygon(mid[0]+Math.sin(yaw)*.02,mid[1]+Math.cos(yaw)*.02,owner.polygon)?1:-1;
  walls.push({id:'wall-'+walls.length,axis:'diagonal',start:a,length,yaw,roomIds:[room],normal,exterior:true,tall:true,openings:[]});
 }
 for(const [kind,x,z,width,ids,name,swing=1,hingeAtEnd=false] of openings){
  const wall=walls.find(w=>{
   const yaw=wallYaw(w),dx=x-w.start[0],dz=z-w.start[1],along=dx*Math.cos(yaw)-dz*Math.sin(yaw),across=dx*Math.sin(yaw)+dz*Math.cos(yaw);
   return ids.every(id=>w.roomIds.includes(id))&&(ids.length>1||w.exterior)&&Math.abs(across)<.01&&along-width/2>=-.001&&along+width/2<=w.length+.001;
  });
  if(!wall)throw Error('Opening does not fit: '+JSON.stringify([kind,x,z,width,ids]));
  wall.openings.push({kind,at:(x-wall.start[0])*Math.cos(wallYaw(wall))-(z-wall.start[1])*Math.sin(wallYaw(wall))-width/2,width,id:ids[0]+'-'+ids.at(-1)+'-'+kind,name:name||rooms.find(r=>r.id===ids.at(-1)).name+' door',swing,hingeAtEnd});
 }
 return walls;
}
export function destinationClear(layout,x,z,roomId,margin=.28,open=false){
 ...
 for(const wall of layout.walls)for(const s of wallSegments(wall))if(s.position[1]-s.size[1]/2<1.6&&circleIntersectsBox(x,z,margin,{x:s.position[0],z:s.position[2],width:s.size[0],depth:s.size[2],yaw:s.yaw}))return false;
...
export function finishLayout({id,area,rooms,openings,furniture,entrance,decor=[],zones=[],theme}){
 const walls=makeWalls(rooms,openings),doors=walls.flatMap(w=>w.openings.filter(o=>o.kind==='door').map(o=>{const at=o.at+(o.hingeAtEnd?o.width:0);return {...o,hinge:wallPoint(w,at),baseAngle:wallYaw(w)+(o.hingeAtEnd?Math.PI:0)};}));
 ...
 const layout={id,theme,rooms,walls,doors,furniture,decor,zones,bounds,APARTMENT:{...defaults,advertisedArea:area.toFixed(2),entrance:{position:[...entrance.position],yaw:entrance.yaw},overview:{position:[cx+16,20,cz+18],target:[cx,.5,cz]}},switches:[]};
