...
async(page)=>{
 ...
 try{
  for(const width of [320,390]){
   await page.setViewportSize({width,height:844});await page.goto(base+'#/projects/tokyo-city');await page.reload();await page.locator('.welcome-dialog .language-picker select').selectOption('en-US');await page.locator('.welcome-dialog .dialog-close').click();await page.locator('.residence-card').first().waitFor();
   check(await page.locator('.residence-card').count()===6,'Missing mobile plans');
   check(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth),'Mobile collection overflow');
   await page.getByRole('button',{name:'3 bedrooms',exact:true}).click();check(await page.locator('.residence-card').count()===1,'Mobile bedroom filter');
   await page.getByRole('button',{name:'All floor plans',exact:true}).click();
   await page.locator('.plan-showcase').screenshot({path:`output/playwright/plans-mobile-${width}.png`});
  }
  for(const [id,area] of [['two-room-euro-52','52.10'],['three-room-euro-82','82.30']]){
   ...
   await page.getByRole('link',{name:'Back to Tokyo City floor plans',exact:true}).click();await page.locator('.residence-card').first().waitFor();check(!await page.locator('canvas').count(),'Viewer retained after exiting');
