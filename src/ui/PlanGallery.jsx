...
export default function PlanGallery({project}){
  ...
  useEffect(()=>{try{localStorage.setItem('artwin-saved-plans',JSON.stringify(saved));}catch{/* Saving is optional. */}},[saved]);
  ...
  const toggleSaved=id=>setSaved(ids=>ids.includes(id)?ids.filter(value=>value!==id):[...ids,id]);
  ...
  return <section className="plan-showcase" aria-labelledby="floor-plans-title">
    ...
    <div className="residence-list">{visible.map(plan=><article className="residence-card" key={plan.id} data-plan-id={plan.id}>
      ...
      <div className="residence-copy"><div className="residence-topline"><span className="collection-kicker">{t('APARTMENT {number}',{number:String(project.plans.indexOf(plan)+1).padStart(2,'0')})}</span><button className={`save-plan ${saved.includes(plan.id)?'saved':''}`} aria-label={`${t(saved.includes(plan.id)?'Remove from saved plans':'Save this plan')} · ${number(plan.area,2)}`} aria-pressed={saved.includes(plan.id)} onClick={()=>toggleSaved(plan.id)}><Icon name="heart"/></button></div><h3>{t(plan.name)}</h3><Metrics plan={plan}/><div className="plan-highlights"><span>{t(plan.id.includes('euro')?'Open living':'Separate kitchen')}</span><span>{t(plan.bedrooms>1?'Family space':'Private loggia')}</span></div><p className="residence-description">{t(plan.description)}</p><div className="residence-actions"><a className="collection-button" href={apartmentHref(project,plan)} aria-label={t('Explore {area} m² in 3D',{area:number(plan.area,2)})}>{t('Explore in 3D')}<Icon name="arrow"/></a><label className="compare-choice" title={t('Compare up to 3 plans')}><input type="checkbox" checked={compare.includes(plan.id)} disabled={compare.length===3&&!compare.includes(plan.id)} onChange={()=>toggleCompare(plan.id)}/>{t('Compare')}</label></div></div>
    </article>)}</div>
