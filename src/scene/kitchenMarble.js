...
export function kitchenMarbleTextures(){
  ...
  for(let y=0;y<1024;y++)for(let x=0;x<1024;x++){
    const seed=47+Math.floor(x/512)*113+Math.floor(y/512)*277,nx=x%512/512,ny=y%512/512;
  ...
  for(const c of [ctx,b]){c.strokeStyle=c===ctx?'#b9b9b2':'#555555';c.lineWidth=2;c.beginPath();for(const n of [0,512,1024]){c.moveTo(n,0);c.lineTo(n,1024);c.moveTo(0,n);c.lineTo(1024,n);}c.stroke();}
  const map=new THREE.CanvasTexture(canvas),bump=new THREE.CanvasTexture(relief);map.colorSpace=THREE.SRGBColorSpace;
  for(const t of [map,bump]){t.wrapS=t.wrapT=THREE.RepeatWrapping;t.repeat.set(2.5,2.5);t.anisotropy=8;}
  return {map,bump};
}
